      ...
      <div class="card">
        <div class="card-body">
          ...
          <div class="search-add">
            <a href="/informasi/renungan/create" class="add">Add Renungan</a>
          </div>
        ...
        <tbody>
            @foreach ($renungans as $data)
                <tr>
                    ...
                    <td>
                        <img src="{{ asset('renungan/' . $data->gambar1) }}" 
                            alt="{{ $data->judul }}" 
                            style="max-width: 150px; max-height: 100px; object-fit: cover;">
                    </td>
                    <td>
                        <form class="d-inline" action="/informasi/renungan/{{ $data->id }}/edit" method="GET">
						...
						@if(auth()->user()->role === 'Admin' || auth()->user()->role === 'Editor')
							...
							@endif
						@endif
                        <form class="d-inline" action="/informasi/renungan/{{ $data->id }}" method="POST">
                            @csrf
                            @method('delete')
...
<script>
    function ubahStatus(status, id) {
        ...
        const url = `/api/update-status-renungan/${id}`;
