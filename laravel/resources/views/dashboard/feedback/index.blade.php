...
@section('title', 'Feedbacks Masyarakat')
      ...
      <div class="card">
        <div class="card-body">
          <h5 class="card-title">@yield('title')</h5>
            ...
            <a href="{{ route('feedback-export.csv') }}" class="add">Export CSV</a>
            </div>
	...
	<table class="table table-bordered border-primarytable-striped table-class" id= "table-id" style="margin-top: 20px;">
        ...
        <tbody>
            @foreach ($feedbacks as $data)
                <tr>
                    <td class="truncate-text"><span style="text-overflow: ellipsis; max-width: 100px;-webkit-line-clamp: 2;">{{ $data->gender }}</span></td>
                    <td class="truncate-text"><span style="text-overflow: ellipsis; max-width: 10px;-webkit-line-clamp: 2;">{{ $data->umur }}</span></td>
                    <td class="truncate-text"><span style="text-overflow: ellipsis; max-width: 10px;-webkit-line-clamp: 2;">{{ $data->pekerjaan }}</span></td>
                    <td>
						<button 
							style="margin: 5px;" 
							type="button" 
							...
							data-title="Feedback User" 
							data-gender="{{$data->gender}}"
							data-umur="{{$data->umur}}"
							data-pekerjaan="{{$data->pekerjaan}}"
							data-pendapat="{{$data->pendapat}}"
		...
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				...
				<div class="modal-body">
					<span><strong>Gender : </strong><span class="gender"></span></span><br>
					<span><strong>Usia : </strong><span class="usia"></span></span><br>
					<span><strong>Pekerjaan : </strong><span class="pekerjaan"></span></span><br>
					<span><strong>Skala Pendapat : </strong><span class="pendapat"></span></span><br>
					<span><strong>Skala Rekomendasi : </strong><span class="rekomendasi"></span></span><br>
					<span><strong>Favorite Menu : </strong><span class="favorite_menu"></span></span><br>
					<span><strong>Saran : </strong><span class="saran"></span></span><br>
				</div>
...
<script>
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		modal.addEventListener('show.bs.modal', function (event) {
			...
			const usia = button.getAttribute('data-umur');
			const pekerjaan = button.getAttribute('data-pekerjaan');
			const pendapat = button.getAttribute('data-pendapat');
			const rekomendasi = button.getAttribute('data-rekomendasi');
			const favorite_memu = button.getAttribute('data-favorite_menu');
			const saran = button.getAttribute('data-saran');

			// Update the modal's title and body content
			modal.querySelector('.modal-title').textContent = title;
			modal.querySelector('.gender').textContent = gender;
			modal.querySelector('.usia').textContent = usia;
			modal.querySelector('.pekerjaan').textContent = pekerjaan;
			modal.querySelector('.pendapat').textContent = pendapat;
			modal.querySelector('.rekomendasi').textContent = rekomendasi;
			modal.querySelector('.favorite_menu').textContent = favorite_memu;
			modal.querySelector('.saran').textContent = saran;
