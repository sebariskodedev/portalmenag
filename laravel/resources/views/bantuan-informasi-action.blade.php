...
<main class="main dinamyc-color">
  ...
  <section id="portfolio" class="portfolio section dinamyc-color">
    <div class="container">
      <div class="article">
        <h1 class="article-title text-dinamyc-color-primary">{{$bantuan->nama}}</h1>
        <p class="article-meta text-dinamyc-color">Dipublikasi pada {{$bantuan->created_at}}</p>

        <div class="article-content">

            <!-- First Image (Top Right) -->
            <figure class="image-square-top top-center-image">
              <img src="{{ asset('bantuan-informasi/' . $bantuan->gambar) }}" alt="Top Right Image">
            </figure>

            <p data-content="{{$bantuan->deskripsi}}" class="post-description text-dinamyc-color deskripsi-berita">The content from Quill will appear here.</p>

            <span class="post-description text-dinamyc-color"><strong>Status Bantuan:</strong> {{$bantuan->status}}<br><strong>Tipe Bantuan:</strong> {{$bantuan->tipe}}<br><strong>Jenis Bantuan:</strong> {{$bantuan->jenis}}<br><strong>Kategori Bantuan:</strong> {{$bantuan->kategori}}<br><strong>Jumlah Kuota:</strong> {{$bantuan->jumlah_kuota}}<br><strong>Periode:</strong> {{$bantuan->tanggal_pembukaan}} - {{$bantuan->tanggal_penutupan}}</span>
            <br>
            @if($bantuan->juknis != null)
            <span class="post-description text-dinamyc-color"><strong>Juknis: </strong> <a style="" href="{{ asset('juknis-bantuan-informasi/' . $bantuan->juknis) }}" download>Download</a></span>
            @else
            <span class="post-description text-dinamyc-color"><strong>Juknis: </strong> -</span>
            @endif
